...
受信した投稿のタグ：
  ...
  結果：非表示（value「2」がユーザーの有効値に含まれない）
...
| `value:` | 規模・程度・重み付け | `5+`, `warning`, `final`, `highlight` |
...
例えば、既存のJMA地震Botが `value:5+` および `target:jp-kanto` を使用している場合、同じくJMA地震データを処理する新しいBotは、同一の `value:5+` および `target:jp-kanto` を使用しなければなりません。`value:震度5強`、`value:shindo-5-upper`、`target:jp-kantou`、`target:jp-関東` などの代替値を使用してはいけません。
...
- **Bot:** [bsaf-jma-bot](https://github.com/osprey74/bsaf-jma-bot) — 日本の地震・津波・気象アラート
- **クライアント:** [kazahana](https://github.com/osprey74/kazahana) — BSAF対応 Blueskyデスクトップクライアント
