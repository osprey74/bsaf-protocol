...
> **Status:** Phase 1 完了 — 仕様 v1 確定、リファレンスBot稼働中、リファレンスクライアント実装済み
...
- kazahanaはBSAFのリファレンスクライアントとして、タグベースのフィルタリング機能を実装済み（Bot登録・動的フィルタUI・重複検知・折りたたみ・self_url更新チェック、全11言語対応）
...
├── README.md          ← リポジトリのメインドキュメント（英語）
├── README-ja.md       ← リポジトリのメインドキュメント（日本語）
├── ROADMAP.md         ← ロードマップ＆デモ計画
...
│   └── ...                ← 調査資料
├── presets/
│   └── jp/            ← 日本向けフィルタプリセット（将来追加予定）
├── examples/          ← Bot・クライアント実装のサンプルコード（将来追加予定）
├── src/               ← バリデーションツール等の共通ユーティリティ（将来追加予定）
...
- N/A `examples/` ディレクトリ — サンプルコードは未作成（Phase 3以降で追加予定）
- N/A `presets/jp/` — プリセットJSONは未作成（Phase 3以降で追加予定）
